...
import 'package:mapa_app/models/search_response.dart' as searchResponse;
...
class SearchDestination extends SearchDelegate<SearchResult> {
  ...
  Widget _construirResultadoSugerencias() {
    ...
    return StreamBuilder(
      ...
      builder: (BuildContext context, AsyncSnapshot<searchResponse.SearchResponse> snapshot) {
        if (!snapshot.hasData) {
        ...
        if (lugares.length == 0) {
          return ListTile(
            title: Text('No hay resultados encontrados')
          );
        }
